...
contextBridge.exposeInMainWorld('zoom', {
	setZoom: (zoomLevel: number): void => {
		if (zoomLevel < 0) {
			console.error("Attempted to change zoom to: " + zoomLevel);
			console.error("Reset to default");
	...
	// onZoomChanged: (callback: (event: any, zoomDirection: string) => void) => {
	// 	webContents.on('zoom-changed' as any, callback);
	// },
})
